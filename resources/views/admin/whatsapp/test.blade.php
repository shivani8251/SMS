                  ...
                  <div class="row">
                    <div class="col-12">
                      <div class="form-group">
                        <label class="form-control-label">Campaign Name (Heading) </label>
                        <input type="text" placeholder="Enter campaign name" id="campaign-name" autocomplete="off" name="campaign_name" class="form-control" />
                      </div>
                    </div>
  ...
  $(function() {
    ...
    $("#send-whatsapp-form").submit(function(event) {
      ...
      if (whatsappForm.valid())
      {
        ...
        $.ajax({
          type: 'POST',
          url: "{{ route('admin.parent.test.send') }}",
          data: formData,
          dataType: 'json',
          contentType: false,
          cache: false,

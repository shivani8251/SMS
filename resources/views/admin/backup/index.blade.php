                          ...
                          <div class="form-group">
                            <select id="user-filter" class="form-control select2 filter-select2" data-placeholder="Select User">
                              ...
                              @foreach($users as $user)
                                <option value="{{$user->id}}">{{ucfirst($user->username).' ('.$user->user_unique_id.')'}}</option>
                              @endforeach
                            </select>
                          </div>
                        </div>
  ...
  $(function() {
    ...
    $(document).on('click', '.delete-campaign-btn', function() {
      ...
      $.confirm({
        ...
        buttons: {
          yes: function() {
            $.get("{{ route('admin.campaign.delete') }}", {id}, function(res) {
              if(res.status)
              {
    ...
    function reportTable(user = 'All', date_range) {
      ...
      var whatsappTable = $("#whatsapp-report-table").DataTable({
        ...
        order: [],
        ajax: {
            url: "{{ route('admin.backup.serverSideDataTable') }}",
            type: "GET",
            data: {
              user: user,
